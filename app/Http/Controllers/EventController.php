...
#FIXME: stop the whole process, it should not create another package and other releavnt functions when all the services are not available in the date mentioned 
        ...
        // Check if services in the package are available
        // Check if services in the package are available
        foreach ($validatedData['packages'] as $packageId) {
            ...
            foreach ($services as $serviceId) {
                ...
                $existingBookings = DB::table('event_services_providers')
                    ->where('service_id', $serviceId)
                    ->whereDate('event_booked_date', '=', $event->event_datetime->format('Y-m-d'))
                    ->count();

                // Ensure events_per_day is properly validated
                $allowedEvents = $service->events_per_day ?? 1; // Default to 1 if not set

                // If bookings exceed the allowed number of events per day, mark the service as unavailable
                if ($existingBookings >= $allowedEvents) {
                    $unavailableServices[] = $service->serviceName;
                }
            }
        }

        // If any service is unavailable, terminate transaction and return error
        if (count($unavailableServices) > 0) {
            DB::rollBack(); // Rollback transaction
            return response()->json([
                'status' => 'error',
                'message' => 'Not all services in the selected package are available on the selected date.',
        ...
        foreach ($validatedData['packages'] as $packageId) {
            ...
            foreach ($services as $serviceId) {
                ...
                // Assuming the event start time is being passed or calculated
                $eventBookedTimeStart = $event->event_datetime->format('H:i'); // Start time (already available)
                
                // Add a default duration to the event's start time (e.g., 2 hours for average service time)
                $eventBookedTimeEnd = Carbon::parse($eventBookedTimeStart)->addHours(6)->format('H:i'); // Add 2 hours
                ...
                DB::table('event_services_providers')->insert([
                    ...
                    'event_booked_date' => $event->event_datetime->format('Y-m-d'), // Use the event's booked date
                    'event_booked_time_start' => $eventBookedTimeStart,  // Start time of the service
                    'event_booked_time_end' => $eventBookedTimeEnd,  // End time of the service (start time + 2 hours)
                    'created_at' => now(),
                    'updated_at' => now(),
                ]);
            }
